package jonathanchiou.csmathhelper.main.Utils;

public class Constants {
    //KEYS
    public static final String MODE_KEY = "MODE";
    public static final String TOTAL_SOLVED_KEY = "TOTAL";
    public static final String TOTAL_SOLVED_BIN_KEY = "BIN_TOTAL";
    public static final String TOTAL_SOLVED_HEX_KEY = "HEX_TOTAL";
    public static final String TOTAL_SOLVED_TEST_KEY = "BOTH_TOTAL";
    public static final String SAVE_KEY = "SAVE";
    public static final String BIN_KEY1 = "BIN1";
    public static final String BIN_KEY2 = "BIN2";
    public static final String BIN_RESULT_KEY = "BINRESULT";
    public static final String BIN_OP = "BINOP";
    public static final String HEX_KEY1 = "HEX1";
    public static final String HEX_KEY2 = "HEX2";
    public static final String HEX_OP = "HEXOP";
    public static final String HEX_RESULT_KEY = "HEXRESULT";
    public static final String MIXED_KEY1 = "MIX1";
    public static final String MIXED_KEY2 = "MIX2";
    public static final String MIXED_OP = "MIXOP";
    public static final String MIXED_MODE_KEY = "MODE2";
    public static final String MIXED_RESULT_KEY = "MIXRESULT";

    //VALUES
    public static final String BINARY_MODE = "BINARY";
    public static final String HEX_MODE = "HEX";
    public static final String MIXED_MODE = "MIXED";

    //RANDO STRING CRAP
    public static final String TOTAL_COUNT_STRING = "Total Questions Solved: ";
    public static final String TOTAL_BIN_CNT_STRING = "Total Binary Questions Solved: ";
    public static final String TOTAL_HEX_CNT_STRING = "Total Hex Questions Solved: ";
    public static final String TOTAL_MIXED_CNT_STRING = "Total Mixed Questions Solved: ";
    public static final String SETTINGS = "SETTINGS";

    //FRAGMENT TAGS
    public static final String HOME = "Home";
    public static final String BINARY = "Binary";
    public static final String HEX = "Hex";
    public static final String MIXED = "Mixed";
    public static final String TIMED = "Timed";

    //Correct/Incorrent display strings
    public static final String CORRECT = "Correct!";
    public static final String WRONG = "Wrong!";
}
